...
const roundTimerInSeconds = 10; // Timer used to countdown when the round is over.
    ...
    roundCountdown.start(
        ...
        () => {            
            ...
            firebase.database().ref(nodes.games).child(gamecode).child(nodes.round).child(nodes.timerActive)
                ...
                .catch((error) => {
                    utils.logError(error, `Error: Failed to set the ${nodes.timerActive} field to false!`);
                });   
        }
    );
}
