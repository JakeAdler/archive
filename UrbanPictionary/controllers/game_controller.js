    ...
    firebase.database().ref(nodes.games).child(reqObj.gamecode).once('value')
        .then((snap) => {
            ...
            if (snap.exists()) {
                ...
                } else if (!utils.getPlayerIfEnrolledInGame(allPlayers, reqObj.playerUuid)) {
        ...
        .then((snap) => {
            ...
            if (snap.exists()) {
                ...
                if (isGameOwner(allPlayers, reqObj.uuid)) {
                    firebase.database().ref(nodes.games).child(reqObj.gamecode).child(nodes.round).child(nodes.timerActive)
                        .set(true)
                        .then(() => {
                        ...
                        .catch((error) => {
                            utils.logError(error, `Error occurred while attempting to toggler the ${nodes.timerActive} flag to true`);
                        });
                ...
                if (reqObj.judgeUuid === getJudgeUuid(allPlayers)){
                    ...
                    gameRef.child(nodes.round).child(nodes.started)
                        ...
                        .then(() => {
                            ...
                            var winnerUid = utils.getPlayerIfEnrolledInGame(allPlayers, reqObj.winnerUuid);
                            ...
                            resetAllPlayerDrawings(allPlayers, reqObj.gamecode);
                        }).catch((error) => {
...
function extractPlayerInfo(players, uuid, property) {
    var playeruid = utils.getPlayerIfEnrolledInGame(players, uuid);
...
function resetAllPlayerDrawings(allPlayers, gamecode){
    Object.keys(allPlayers).map((uid) => {
        resetPlayerDrawing(uid, gamecode);
    });
}

function resetPlayerDrawing(playerUid, gamecode){
    firebase.database().ref(nodes.games).child(gamecode).child(nodes.players).child(playerUid).child(nodes.drawing)
        .set("*")
        .catch((error) => {
            utils.logError(error, "Error: Failed to reset the player's drawing");
        });
}

function getAllDrawersUUIDs(allPlayers){
    return Object.keys(allPlayers).filter((uid) => {
        return !allPlayers[uid].judge;
    }).map((uid)=>{
        var player = allPlayers[uid];
        return {
            uuid: player.uuid,
